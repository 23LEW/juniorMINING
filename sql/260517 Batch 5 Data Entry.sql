-- Batch 5: 5 Diverse Success-Companies - DATA ENTRY
-- Full data from research: Pretium, Detour, Sabina, Great Bear, Osisko
-- Schema Version: v1.3
-- Date: 2026-05-17
...
-- ============================================================
-- COMPANY 1: PRETIUM RESOURCES INC.
-- ============================================================
-- Commodity: Au + Ag
-- Exit Type: M&A
-- Exit Year: 2022
...
VALUES ('Pretium Resources Inc.', NULL, 'Canada', 'TSX', 2010, 2022, 'Au', 0, 'success');

-- Pretium Resources Persons
INSERT INTO person (vorname, nachname, education, country, production_ramp_up_experience, first_mention_year, bio_text)
VALUES ('Jacques', 'Perron', NULL, 'Canada', 1, 2020, 'CEO Pretium Resources (2020-2022), led company through Newcrest acquisition');

INSERT INTO person (vorname, nachname, education, country, production_ramp_up_experience, first_mention_year, bio_text)
VALUES ('Matthew', 'Quinlan', NULL, 'Canada', 1, 2020, 'CFO Pretium Resources (2020-2022), financial lead in M&A transaction');

-- Pretium Resources Roles
INSERT INTO role (person_id, company_id, role_type, start_date, end_date)
SELECT (SELECT id FROM person WHERE vorname='Jacques' AND nachname='Perron' LIMIT 1),
       (SELECT id FROM company WHERE name='Pretium Resources Inc.' LIMIT 1),
       'CEO', '2020-04-01', '2022-03-09';

INSERT INTO role (person_id, company_id, role_type, start_date, end_date)
SELECT (SELECT id FROM person WHERE vorname='Matthew' AND nachname='Quinlan' LIMIT 1),
       (SELECT id FROM company WHERE name='Pretium Resources Inc.' LIMIT 1),
       'CFO', '2020-09-01', '2022-03-09';

-- Pretium Resources Project
INSERT INTO project (company_id, name, jurisdiction, primary_commodity, stage_at_acquisition, peak_stage)
SELECT (SELECT id FROM company WHERE name='Pretium Resources Inc.' LIMIT 1),
       'Brucejack', 'British Columbia, Canada', 'Au', 'Production', 'Production';

-- Pretium Resources Outcome
-- Score: 0.25*85 + 0.25*95 + 0.30*100 + 0.20*90 = 21.25 + 23.75 + 30 + 18 = 93
...
SELECT (SELECT id FROM company WHERE name='Pretium Resources Inc.' LIMIT 1),
       85, 95, 100, 90, 93, 'M&A', 2022, 3500;

-- Pretium Resources Events
INSERT INTO event (company_id, event_type, event_date, description)
SELECT (SELECT id FROM company WHERE name='Pretium Resources Inc.' LIMIT 1),
       'Production Start', '2017-07-01', 'Brucejack Gold Mine achieves commercial production';

INSERT INTO event (company_id, event_type, event_date, description)
SELECT (SELECT id FROM company WHERE name='Pretium Resources Inc.' LIMIT 1),
       'M&A', '2022-03-09', 'Newcrest Mining completes acquisition of Pretium Resources for CAD 3.5B';
...
-- Detour Gold Outcome
-- Score: 0.25*80 + 0.25*90 + 0.30*100 + 0.20*100 = 20 + 22.5 + 30 + 20 = 92.5
INSERT INTO outcome (company_id, discovery_score, reserve_conversion_score, exit_production_score, peak_marketcap_score, total_score, exit_type, exit_year, peak_marketcap_cad_million)
SELECT (SELECT id FROM company WHERE name='Detour Gold Corp.' LIMIT 1),
...
SELECT (SELECT id FROM company WHERE name='Sabina Gold & Silver Corp.' LIMIT 1),
       'Back River (Goose Mine)', 'Nunavut, Canada', 'Au', 'Construction', 'Development';

-- Sabina Gold & Silver Outcome
-- Score: 0.25*75 + 0.25*85 + 0.30*90 + 0.20*80 = 18.75 + 21.25 + 27 + 16 = 83
INSERT INTO outcome (company_id, discovery_score, reserve_conversion_score, exit_production_score, peak_marketcap_score, total_score, exit_type, exit_year, peak_marketcap_cad_million)
SELECT (SELECT id FROM company WHERE name='Sabina Gold & Silver Corp.' LIMIT 1),
...
SELECT (SELECT id FROM company WHERE name='Great Bear Resources Ltd.' LIMIT 1),
       'Dixie Project', 'Red Lake District, Ontario, Canada', 'Au', 'Discovery', 'Development';

-- Great Bear Resources Outcome
-- Score: 0.25*90 + 0.25*85 + 0.30*75 + 0.20*85 = 22.5 + 21.25 + 22.5 + 17 = 83.25
INSERT INTO outcome (company_id, discovery_score, reserve_conversion_score, exit_production_score, peak_marketcap_score, total_score, exit_type, exit_year, peak_marketcap_cad_million)
SELECT (SELECT id FROM company WHERE name='Great Bear Resources Ltd.' LIMIT 1),
...
SELECT (SELECT id FROM company WHERE name='Osisko Mining Inc.' LIMIT 1),
       'Windfall Gold Project', 'Lebel-sur-Quevillon, Quebec, Canada', 'Au', 'Development', 'Development';

-- Osisko Mining Outcome
-- Score: 0.25*95 + 0.25*90 + 0.30*85 + 0.20*95 = 23.75 + 22.5 + 25.5 + 19 = 90.75
INSERT INTO outcome (company_id, discovery_score, reserve_conversion_score, exit_production_score, peak_marketcap_score, total_score, exit_type, exit_year, peak_marketcap_cad_million)
SELECT (SELECT id FROM company WHERE name='Osisko Mining Inc.' LIMIT 1),
...
--
-- Next steps:
-- 1. Execute this SQL locally with: sqlite3 junior_mining.db < 260517_Batch5_Data_Entry.sql
-- 2. Verify with: SELECT COUNT(*) FROM company WHERE success_label='success';
-- 3. Check specific data: SELECT * FROM outcome WHERE exit_year >= 2020;
-- 4. Run verification queries from 260517 Batch 5 Audit.md
-- ============================================================
