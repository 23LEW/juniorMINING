#!/usr/bin/env python3
"""
Batch 5 Database Insert Script
...
def insert_batch5():
    """Insert Batch 5 data into database"""

    print("=" * 70)
    print("BATCH 5 DATABASE INSERT")
    print("=" * 70)
    ...
    try:
        cursor.execute("""
            ...
            WHERE c.name IN ('Pretium Resources Inc.', 'Detour Gold Corp.',
                            'Sabina Gold & Silver Corp.', 'Great Bear Resources Ltd.',
                            'Osisko Mining Inc.')
            ORDER BY c.name
        """)
        ...
        for row in results:
            ...
            print(f"[ID {row[0]:3d}] {row[1]:<35} | {row[2]}-{row[3]} {row[4]:<4} | {row[5]:<6} {row[6]} | Score: {row[7]:.1f}")
    except sqlite3.Error as e:
